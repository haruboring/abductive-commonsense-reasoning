...
from .utils import bert_cos_score_idf, bert_types, get_bert_embedding, get_idf_dict
...
def score(cands, refs, bert="bert-base-multilingual-cased", num_layers=8, verbose=False, no_idf=False, batch_size=64):
    ...
    all_preds = bert_cos_score_idf(
        model, refs, cands, tokenizer, idf_dict, verbose=verbose, device=device, batch_size=batch_size
    )
...
def plot_example(h, r, verbose=False, bert="bert-base-multilingual-cased", num_layers=8, fname=""):
    """
    BERTScore metric.
    Args:
    ...
    ref_embedding, ref_lens, ref_masks, padded_idf = get_bert_embedding([r], model, tokenizer, idf_dict, device=device)
    hyp_embedding, ref_lens, ref_masks, padded_idf = get_bert_embedding([h], model, tokenizer, idf_dict, device=device)
    ...
    for i in range(len(h_tokens)):
        for j in range(len(r_tokens)):
            text = ax.text(
                j, i, "{:.3f}".format(sim[i, j]), ha="center", va="center", color="k" if sim[i, j] < 0.6 else "w"
            )
