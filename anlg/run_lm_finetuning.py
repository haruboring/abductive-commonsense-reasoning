...
restricted_comet_relations = {"obs1": ["xEffect", "xWant", "xReact"], "obs2": ["xIntent", "xNeed"]}
...
def anli_record_to_gpt_prompt(tokenizer: AnliGpt2Tokenizer, record: dict, is_eval: bool = False):
    context = [record["obs1"], record["obs2"], "Because,  "]
...
def record_to_text_tokens_with_comet_pred(
        ...
        for obs in ["obs1", "obs2"]:
            for category in all_categories:
                ...
                if comet_as_text:
                    ...
                    if (
                        category in record["comet_preds"][obs]
                        and record["comet_preds"][obs][category]["beams"][0] != "none"
                    ):
    ...
    if is_eval:
        ...
        training_instance = context + [record["hyp" + record["label"]], tokenizer.eexpl_token]
...
class AnliDataset(Dataset):
    def __init__(
        ...
        comet_data_loader=None,
        comet_as_text=False,
        conditional_lm=False,
        restrict_comet=False,
        no_cache=False,
        is_eval=False,
        sotw=False,
    ):
        ...
        if os.path.exists(cached_features_file):
            logger.info("Loading features from cached file %s", cached_features_file)
            with open(cached_features_file, "rb") as handle:
                self.examples, self.labels, self.comet_inputs, self.comet_masks = pickle.load(handle)
        else:
            logger.info("Creating features from dataset file at %s", directory)
            ...
            for record in tqdm(records, "Encoding Data"):
                text_tokens, comet_event_inputs, comet_attention_masks = record_to_text_tokens_with_comet_pred(
                    tokenizer=tokenizer,
                    record=record,
            ...
            if not no_cache:
                ...
                with open(cached_features_file, "wb") as handle:
                    pickle.dump(
                        (self.examples, self.labels, self.comet_inputs, self.comet_masks),
                        handle,
...
def load_and_cache_anli_examples(
    args, tokenizer, evaluate=False, include_comet=False, comet_text_encoder=None, comet_data_loader=None, sotw=False
):
    dataset = AnliDataset(
        tokenizer,
...
def evaluate(args, model, tokenizer, evaluate=False, comet_text_encoder=None, comet_data_loader=None, prefix=""):
    ...
    if args.task is None:
        ...
        eval_dataset = load_and_cache_anli_examples(
            args,
            tokenizer,
            evaluate=True,
    ...
    if len(prefix) == 0:
        results.update({"perplexity": perplexity.item(), "eval_loss": eval_loss})
    else:
        results.update({"perplexity_{}".format(prefix): perplexity.item(), "loss_{}".format(prefix): eval_loss})
...
def main():
    ...
    parser.add_argument(
        "--eval_output_dir", default=None, type=str, required=False, help="Directory to write results to"
    )
    parser.add_argument("--tb_dir", default=None, type=str, required=False, help="Directory to write tensorboard to")

    ## Other parameters
    parser.add_argument(
        "--task", default=None, type=str, help="The task to finetune the LM on. Currently supports None / anli"
    )
    ...
    parser.add_argument("--conditional_lm", default=False, type=bool, help="Comet feature encoded using text")
    parser.add_argument(
        "--restrict_comet", default=False, type=bool, help="Restrict comet features to only o1's effect and o2's causes"
    )
    parser.add_argument("--sotw", default=False, type=bool, help="Use the state of the world model.")
    parser.add_argument(
        "--no_cache", default=False, type=bool, help="Restrict comet features to only o1's effect and o2's causes"
    )
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.include_comet and not args.comet_as_text:
            ...
            # print(opt)
            comet_data_loader, comet_text_encoder = interactive.load_data("atomic", opt, vocab, args.comet_vocab_path)
